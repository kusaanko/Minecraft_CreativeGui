    ...
    public void scrollTo(float par1) {
        ...
        if (GuiContainerCreativeGui.getSelectedIndex() != -2) {
            for (int i = 0; i < 5; ++i) {
                for (int l = 0; l < 9; ++l) {
                    this.addSlot(
                            new Slot(GuiContainerCreativeGui.getInventory(), l + (i * 9), 9 + l * 18, 48 + i * 18));
                }
            }
        } else {
            for (int y = 0; y < 3; ++y) {
        ...
        CreativeTabs tab = GuiContainerCreativeGui.getSelectedIndex() == -1 ? CreativeTabs.tabSearch
                : CreativeTabs.tabs.get(GuiContainerCreativeGui.getSelectedIndex());
        ...
        for (int var4 = 0; var4 < 5; ++var4) {
            for (int var5 = 0; var5 < 9; ++var5) {
                ...
                if (var6 >= 0 && var6 < tab.items.size()) {
                    if (tab.items.get(var6) == null)
                        GuiContainerCreativeGui.getInventory().setInventorySlotContents(var5 + var4 * 9, null);
                    else
                        GuiContainerCreativeGui.getInventory().setInventorySlotContents(var5 + var4 * 9,
                                tab.items.get(var6));
                } else {
                    GuiContainerCreativeGui.getInventory().setInventorySlotContents(var5 + var4 * 9, null);
                }
