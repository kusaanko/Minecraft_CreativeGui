	...
	public mod_CreativeGui() {
		Loader.log.addHandler(new Handler() {
			...
			public void publish(LogRecord record) {
				...
				String modName = "";
				if(message.contains(" ")) modName = message.split(" ")[1];
				if(modName.endsWith(".modsLoaded()")) modName = modName.substring(0, modName.length()-13);
				if(modName.endsWith(".load()")) modName = modName.substring(0, modName.length()-7);
			...
			public void update() {
				for(Item item : Item.itemsList) {
					if(item!=null&&!itemMods.containsKey(item.shiftedIndex)) {
						...
						if(modclazz.startsWith("mod_")) modclazz = this.modName.substring(4);
						if(item.shiftedIndex<Block.blocksList.length&&Block.blocksList[item.shiftedIndex]!=null) {
							...
							if(MCAPI.getModName(b).equals("")) name = modclazz;
							MCAPI.setModName(b, name);
						}
						Item i = Item.itemsList[item.shiftedIndex];
						if(MCAPI.getModName(i).equals("")) MCAPI.setModName(i, modclazz);
					}
				}
			}
